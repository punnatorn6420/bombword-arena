...
export async function POST(request: Request, { params }: { params: Promise<{ roomId: string }> }) {
  try {
    ...
    if (!isGameMode(mode)) {
      return NextResponse.json({ message: "โหมดเกมไม่ถูกต้อง" }, { status: 400 });
    }
    ...
    if (room.hostId !== playerId) {
      return NextResponse.json({ message: "เฉพาะคนสร้างห้องเท่านั้นที่เริ่มเกมได้" }, { status: 403 });
    }
    ...
    const message = error instanceof Error ? error.message : "เริ่มเกมไม่สำเร็จ";
    console.error(error);
